...
function SelectLabel({
	...
	return (
		<SelectPrimitive.GroupLabel
			...
			className={cn(
				"text-muted-foreground px-4 py-3 text-xs font-black uppercase tracking-[0.2em] border-b border-border/10 mb-2",
				className,
			)}
			{...props}
		/>
...
function SelectItem({
	...
	return (
		<SelectPrimitive.Item
			...
			className={cn(
				"focus:bg-accent focus:text-accent-foreground not-data-[variant=destructive]:focus:**:text-accent-foreground relative flex w-full cursor-default items-center gap-3 rounded-none py-4 md:py-2.5 pr-8 pl-4 md:pl-3 text-base md:text-xs font-bold md:font-medium outline-hidden select-none data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 *:[span]:last:flex *:[span]:last:items-center *:[span]:last:gap-2",
				className,
			)}
			{...props}
		>
