...
export function AuctionsPage() {
  ...
  const badgeItems = useMemo<FilterBadgeItem[]>(() => {
    ...
    if (filters.minPrice || filters.maxPrice) {
      items.push({
        ...
        onRemove: () => patchFilters({ minPrice: undefined, maxPrice: undefined }),
      });
    }
  ...
  return (
    <MarketplaceLayout
      ...
      onToggleFilters={() => {}}
      onResetFilters={() => patchFilters({ q: "", minPrice: undefined, maxPrice: undefined, sortBy: "createdAt", sortOrder: "DESC" })}
      ...
      toolbar={
        <div className="flex items-center gap-2 md:gap-3 flex-1 justify-end">
          ...
          <div className="hidden md:flex items-center bg-muted/20 border border-border/10 p-0.5 rounded-none h-10">
            <SortSelect className="h-full rounded-none border-0 bg-transparent ring-0 focus:ring-0 gap-2 px-3 text-xs font-medium w-[140px]" />
          </div>
        </div>
      }
      mobileFilters={
        <div className="space-y-4">
          ...
          <SortSelect className="h-11 rounded-none border-border/40 text-[10px] uppercase font-bold" />
        </div>
      }
      content={
