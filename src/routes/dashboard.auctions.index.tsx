...
function SupplierAuctionsPage() {
  ...
  const {
    ...
    company?.id ? { companyId: company.id, page, limit } : { limit },
    { skip: !company?.id },
  );
        ...
        {auctionsResult?.meta && auctionsResult.meta.totalPages > 1 && (
          <div className="flex items-center justify-between border-t border-border px-6 py-4 bg-muted/5">
            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">
              Showing page {auctionsResult.meta.page} of{" "}
              {auctionsResult.meta.totalPages}
            </p>
            <div className="flex gap-2">
              ...
              <Button
                variant="outline"
                size="sm"
                disabled={page >= auctionsResult.meta.totalPages || isFetching}
